...
public class MesContratsService
{
	...
	public MesContratsDTO getMesContrats(Long userId)
	{
		...
		// On récupère d'abord la liste de tous les modeles de contrats
		...
		Query q = em.createQuery("select mc from ModeleContrat mc " +
				"WHERE mc.etat=:etat and mc.dateFinInscription>=:d " +
		...
		computeNewContrat(em, res, mcs, contrats);

		computeExistingContrat(em, res, contrats);
	...
	private void computeNewContrat(EntityManager em, MesContratsDTO res, List<ModeleContrat> mcs, List<Contrat> contrats)
	{
		for (ModeleContrat mc : mcs)
		{
			if (isReallyNew(mc, contrats))
			{
				...
				dto.nomProducteur = mc.getProducteur().getNom();
				...
				dto.nbProduit = summaryDTO.nbProduit;
	...
	private void computeExistingContrat(EntityManager em, MesContratsDTO res, List<Contrat> contrats)
	{
		for (Contrat contrat : contrats)
		{
			if (isHistorique(contrat,em)==false)
			{
